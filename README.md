...
The catalog contains **200 distinct error supports, 189 distinct syndromes, 186 syndrome geometries up to boundary-preserving symmetries, and 22 paired studies**. Cases 1–100 retain their original numbers, stable IDs, exact inputs, and mathematical results. The additional cases are organized as follows:
...
| 101–125 | Growth geometry | How do growth restriction, delayed mergers, separate components, and peeling overhead interact? |
| 126–150 | Likelihood landscapes | What can full class spectra reveal beyond minimum weights and their multiplicities? |
| 151–175 | Boundary and residual geometry | How do boundary contacts, residual components, and cancellation patterns affect logical recovery? |
| 176–200 | Local decision sensitivity | Which one-qubit changes alter correction paths, forest agreement, or logical outcomes? |

This is a curated regression dataset for inspecting mechanisms, reproducing individual cases, and testing implementation changes. Cases were deliberately selected for their behavior; their frequencies, failure fraction, and apparent difficulty do not estimate a logical error rate or comparative decoder performance. All 200 use the same distance-5, Z-only, perfect-syndrome, single-round model with uniform unit growth costs. The catalog does not introduce measurement faults, circuit-level correlations, repeated rounds, or a larger code distance.

Use the Collection filter to isolate the new cases 101–200, the original cases 1–100, or all 200. Combine this with concept, verified-behavior, and BFS-outcome filters and text search. Sort by case number, correction weight, peeling overhead, defect count, closeness to 50% conditional failure, or signed class gap. The table displays 25 cases per page. Table metrics always describe default BFS; the simulator and its individual experiment export follow the selected forest. The table's probability column uses the fixed independent-Z prior p=0.10, separately from the adjustable confidence panel.

Every new case 101–200 has a syndrome geometry absent from the original 100 and every other addition, even after left/right reflection, top/bottom reflection, or a half-turn. This rules out additions made only by stabilizer dressing, logical dressing, or those geometric symmetries. The original catalog keeps its intentional same-syndrome and symmetry pairs. This is a precise input-diversity guarantee, not a claim that all 100 additions represent fundamentally different failure mechanisms.

Each case record exposes its stable ID, exact error support, selection rationale, provenance, and error/syndrome/geometry fingerprints. The fingerprints are bit-mask encodings of the fixed qubit/check ordering, not cryptographic hashes: the error key has 11 hexadecimal digits and the syndrome key has five. The five-digit `syndromeOrbitKey` is the smallest syndrome mask under identity and the three boundary-preserving transformations. Quarter-turns are excluded because they exchange rough and smooth boundary types. Exact input supports are authoritative. Seeded provenance records the sampling construction and any documented transformation; paired records identify controlled one-qubit, stabilizer, logical, complement, or geometric relationships where applicable. Geometry transformations preserve the rough and smooth boundary types.

`catalog-reference.json` is the deterministic full dataset, using schema `decoder-lab.catalog.v1`. It includes the model and indexing conventions, patch geometry, all 200 case records, provenance, pair metadata, BFS corrections and residual witnesses, all three forest results, fully grown edges, global and grown-region class minima with multiplicities, exact class weight spectra, and conditional failure probabilities at p=0.01, 0.05, 0.10, and 0.15. Qubit and stored stabilizer indices are zero-based; a displayed stabilizer S_i uses the stored index plus one. Absolute class parity is intersection with the fixed logical-X cut.

**Export matches · JSON** preserves the matching research records with their model context; **Export matches · CSV** provides a compact analysis table. Both export every matching case across all pages in the current sort order. CSV columns include stable IDs, category/tags, fingerprints, error/syndrome/correction/residual supports, logical outcome, weights, growth and reactivation counts, signed gap, peeling and growth penalties, successful-class feasibility, forest sensitivity, both absolute-class minima and multiplicities, probabilities at the four reference priors, partner ID, selection method/criterion, geometry fingerprint, and sensitivity-study kind. Use JSON for the full geometry, provenance, spectra, per-forest supports, and any documented one-qubit or full-neighbor sensitivity study. A study’s metadata describes default BFS; the ordinary experiment fields follow the selected forest. CSV fields are quoted and embedded quotes/newlines are escaped.
...
- Two hundred surface-code cases, including 22 paired studies, plus six original mechanics experiments.
...
Run `node surface-catalog.test.js` for the 200-case research corpus. It checks stable IDs/order, unique error supports, the 176 additions' expected fields and independently reconstructed seeded inputs, and all 22 paired studies. Its 600 case/forest checks independently form check incidence, logical-cut parity, and stabilizer witnesses. It also verifies global and grown-support references, exact spectrum totals and minimum multiplicities, direct conditional-probability sums, fingerprint encodings, all 100 new syndrome-orbit exclusions, a pinned digest preserving the first 100 records, documented sensitivity counterfactuals, deterministic dataset rebuilding, and all 200 CSV records including quoted-field round trips. Paired checks cover one-qubit differences, logical/stabilizer invariance, complements, and boundary-preserving symmetries.

The browser release checklist covers all 200 cases across three forest strategies and seven views, cluster jumps, reference diagrams, editing and equivalence transforms, sampling, individual and catalog exports, shared/invalid URLs, the existing mechanics view, and both themes at 320–1440px. Additional flows cover the likelihood crossover, same-syndrome posterior invariance, endpoint behavior, q16 neighbor/restore, cancellation marks, delayed reactivation, v3 experiment exports, and prior-preserving/legacy/invalid links. Catalog checks include combined filters, sorting, pagination, exports spanning multiple pages, and opening a selected case. Production-readiness checks cover keyboard details and tabs, lattice and forest focus, search/filter empty states, copy fallback, invalid-link notices, no-JavaScript behavior, reduced motion, and sticky section navigation.
